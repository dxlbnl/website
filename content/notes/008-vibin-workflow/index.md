...
lede: LLMs are good at individual tasks. Getting them to do multi-step projects coherently is a different problem. Here's the pipeline I built.
---

I've been running LLM agents on my projects for a while. They can handle multi-step work; that's not the issue. The issue is verifying that the *right* decisions are getting made, and making sure each new session picks up with the full context of everything that came before it.
...
Single-step prompts work fine. Ask an LLM to refactor a function, it refactors the function. Ask it to write a test, it writes a test. The problems show up when you chain these into a project: output of one step becomes input for the next, across multiple sessions, across multiple agents that don't share context.

The failure mode isn't usually bad code. It's undocumented assumptions. An agent makes a call that's locally reasonable, the next agent inherits it without knowing, and the project accumulates decisions that nobody consciously made. You can't review what wasn't written down.

The solution is a durable, readable record of what was decided and why, one that outlives any individual context window.

---

## The wiki as the single source of truth

The core of Vibin is a `wiki/` directory that lives in the repo alongside the code. Every agent reads `wiki/INDEX.md` before doing anything else. A `PreToolUse` hook enforces this; agents that try to write before reading the wiki get blocked.
...
- **A decision log**: append-only, ADR-style, notable choices only
- **A progress journal**: what the agents have actually done, in order

When code and wiki disagree, the wiki wins. If an agent produces something that contradicts the wiki, you don't patch the code; you figure out whether the spec was wrong and update it there. This keeps the system's memory coherent.
...
- **Scope stays bounded**: anything that surfaces mid-run but is out of scope for the current item goes into `inbox/` as a new card. It doesn't disappear, and it doesn't derail what's in flight.

Each backlog item is a small markdown file with a `type:` field (feature, bug, research, chore) and an optional `flags:` list. Flagging something `review` pauses the pipeline after the spec is written and waits for sign-off before tests get written. For routine work you skip the flag and let it run. For anything risky or genuinely unclear, you add it and read the plan first.
...
The spec-writer step is where planning happens. Not a back-and-forth conversation: the spec-writer reads the backlog item and the wiki and produces a spec page with acceptance criteria, constraints, and a proposed approach. That file is the plan. Add `flags: [review]` to the backlog item and the pipeline pauses after the spec is written, waiting for sign-off before tests start. Leave the flag off and it keeps going. The spec exists either way, sitting in the repo.

The test-first requirement for features and bugs serves a specific purpose in agent pipelines. The test is the spec expressed as runnable code. When the implementer has a failing test to satisfy, it can't hallucinate its way to "done." The test either passes or it doesn't. That binary is much harder to fudge past than "does this look reasonable to you."
...
Before implementing anything I don't already know well, I spin up a research track. The researcher agent is given a topic and writes its findings into `wiki/research/<topic>.md`. The implementer reads that page before writing a line of code.
...
**Phase 1: Design analysis.** I used Claude to analyze [the existing site](/notes/006-site-design/): the CSS variables, component patterns, the visual language (the phosphor aesthetic). The output was a design spec with a token system, component inventory, and a list of variants. That lived in the wiki as the source of truth for what the library should contain.
...
**Phase 3: Site migration.** Once the library was publishing to npm, I ran the pipeline on a regression workflow to migrate the Dexterlabs site page by page. Each page got its own regression report: a diff of what changed, what visual behavior was preserved, what needed a fix. The regression-workflow file in `wiki/` explains the exact steps; `wiki/regressions/` holds the per-page reports.
...
Before I trusted this on the site refactor, I ran the same pipeline on a few toy projects: small TypeScript utilities and a homelab tool. The point was to verify the workflow didn't only work because it was shaped around one specific codebase.
...
The wiki compound interest was real. I expected it to be useful as documentation. I didn't expect agents to produce noticeably better work when they had a well-maintained wiki to read from, but they do. An agent that has read a two-page architecture doc and a three-page component API research page makes fewer wrong assumptions than one working from scratch.

The other thing: the two-agent loop catches a class of error that a single agent reliably misses. Spec-writer and reviewer have no shared session context. When the reviewer says "this doesn't match the spec," it's reading the spec fresh. That independence is worth something.

---

## What still needs work

It does take longer than just asking one agent to implement something. The extra steps are real time. The tradeoff is that I can review direction at each stage: spec before any code is written, tests before the implementation starts. Finding a wrong assumption at the spec stage is cheap. Finding it after the implementer has gone three files deep is not.
...
Yes, mostly. The interesting part is that the LLM both writes the spec and implements against it, with different agents handling each role. A spec written by a spec-writer agent, given a clear backlog item and a well-maintained wiki, is often more complete than what I'd write manually.
...
The failure mode is a spec-writer that writes confident specs around the wrong thing. The reviewer catches some of this since it reads the spec fresh. It can flag "this was never right to begin with," not just "this doesn't match the code."

Without the wiki, each session starts from scratch. The agents do work, but none of it compounds.
